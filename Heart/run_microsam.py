...
"""
MicroSAM inference script for all channels.
...
OUTPUT_DIR = HEART_DATA_ROOT / "benchmark_results" / "microsam_predictions"
MODEL = "vit_b_lm"
DOWNSCALE = 0.5  # Downscale factor for speed
...
def to_u8_robust(img01):
    """Convert [0, 1] float images to uint8 with a fallback for flat ranges."""
    if img01.size == 0:
        return img01
...
def normalize01(img_f32):
    """Robustly normalize floats to [0, 1] using the 1st and 99th percentiles."""
...
def masks_to_label(masks, shape_hw):
    """Convert multiple binary masks into an instance label map."""
...
for tif_path in tqdm(tif_files, desc="MicroSAM", unit="img"):
    ...
    # Ensure inputs are 2D grayscale images
    if image.ndim == 3:
        image = image[..., 0]
