#!/usr/bin/env python3
"""Pooled (section-agnostic) 1-vs-rest Random Forest with repeats and feature importance."""
import argparse
...
def main():
    ap = argparse.ArgumentParser(description="Pooled RF OVR with repeats.")
    ...
    ap.add_argument("--shuffle_labels", action="store_true", help="Leakage check: random label permutation.")
    args = ap.parse_args()
    ...
    imp_accum = {ct: [] for ct in labels}

    rng = np.random.default_rng(args.random_state)
    base_y = df[label_col].astype(str).values
    if args.shuffle_labels:
        df["_label_for_training"] = rng.permutation(base_y); labcol = "_label_for_training"
    else:
        labcol = label_col
    ...
    for r in range(args.n_repeats):
        ...
        for target in labels:
            ...
            clf = make_clf(seed); clf.fit(Xtr, ytr)
            ...
            if args.save_feature_importance:
                rf = clf[-1]; imp = rf.feature_importances_.copy()
                if imp.shape[0]==len(feat_cols): 
                    imp_accum[target].append(imp)

            if args.save_last_preds and r==args.n_repeats-1:
                last_preds.append(pd.DataFrame({
                    "cell_type":target,
                    "image_id":te[group_col].astype(str).values,
                    "roi_id":te[roi_col].astype(str).values,
    ...
    pd.DataFrame(per_rep).to_csv(outdir/"metrics_per_repeat.csv", index=False)
    summarize_auc(pd.DataFrame(per_rep)).to_csv(outdir/"metrics_summary.csv", index=False)
    ...
    if args.save_feature_importance:
        rows=[]
        # Aggregate importance across repeats
        for ct, lst in imp_accum.items():
            if not lst: 
                continue
            arr = np.vstack(lst)
            mu, sd = arr.mean(axis=0), (arr.std(axis=0, ddof=1) if arr.shape[0]>1 else np.zeros(arr.shape[1]))
            for i,f in enumerate(feat_cols):
                rows.append({"cell_type":ct,"feature":f,"mean_importance":float(mu[i]),"std_importance":float(sd[i]),"n_valid":int(arr.shape[0])})
        if rows: 
            pd.DataFrame(rows).to_csv(outdir/"feature_importance_summary.csv", index=False)

    print(f"[OK] per-repeat -> {outdir/'metrics_per_repeat.csv'}")
    print(f"[OK] summary    -> {outdir/'metrics_summary.csv'}")
    if args.save_last_preds: print(f"[OK] last preds -> {outdir/'predictions_last_repeat.csv'}")
    if args.save_feature_importance: print(f"[OK] FI summary-> {outdir/'feature_importance_summary.csv'}")
