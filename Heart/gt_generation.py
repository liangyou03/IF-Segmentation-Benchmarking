...
def read_roi_zip(zip_path):
    """Read ROI coordinates from an ImageJ ROI zip archive."""
    rois = []
...
def parse_roi(roi_bytes):
    """Parse the ImageJ ROI format and handle several ROI types."""
    if len(roi_bytes) < 64:
        return None
...
def create_mask_from_rois(rois, img_shape):
    """Create a mask from ROI polygons, assigning each nucleus a unique ID."""
...
def process_heart_annotations(base_dir, output_dir):
    """Process annotations for all heart regions while preserving directory layout."""
    ...
    stats = []
    mapping = []  # Store mapping information
...
if __name__ == "__main__":
    ...
    output_dir = HEART_DATA_ROOT / "ground_truth_masks"
