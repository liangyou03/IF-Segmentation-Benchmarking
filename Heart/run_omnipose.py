...
OUTPUT_DIR = HEART_DATA_ROOT / "benchmark_results" / "omnipose_predictions"
...
def main() -> None:
    ...
    for img_path in find_images(PROCESSED_DIR):
        try:
            ...
            # Derive output file path while preserving the relative structure
            rel_path = img_path.relative_to(PROCESSED_DIR)

            # Output directory (e.g., OUTPUT_DIR / RA)
            out_dir = OUTPUT_DIR / rel_path.parent
            out_dir.mkdir(parents=True, exist_ok=True)

            # Output filename that matches the _pred.npy convention
