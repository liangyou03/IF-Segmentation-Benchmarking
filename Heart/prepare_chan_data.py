...
"""
Prepare the heart dataset by extracting all channels and organizing files.
Uses ``tifffile`` to read multi-page TIFFs.
...
def load_multichannel_tiff(image_path):
    """Load a multi-channel TIFF using tifffile."""
...
def split_channels(image_path, output_dir, area_name):
    """Split a multi-channel image into separate TIFF files.

    Naming convention: ``{area}_{channel}.tif`` (e.g., ``LA1_dapi.tif``).
    """
    # Load the image
    ...
    for ch_idx, ch_info in CHANNELS.items():
        ...
        # Output filename: {area}_{channel}.tif
...
def main():
    ...
    # Derive all unique images (one per area)
    ...
    # Build a complete mapping file that links to ground truth
    ...
    for _, data_row in data_info_df.iterrows():
        ...
        # Create mappings for each cell type
