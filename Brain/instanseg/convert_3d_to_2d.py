...
DIRS_TO_CONVERT = [
    ...
    Path("/ihome/jbwang/liy121/ifimage/013_ins/refilter_outputs"),
]


def convert_mask(mask: np.ndarray) -> np.ndarray:
    """
    Convert 3D/4D mask to 2D.
    Handles: (1, 1, H, W), (C, H, W), (H, W, C), (H, W)
    """
...
def process_directory(dir_path: Path) -> tuple[int, int, int]:
    ...
    for f in npy_files:
        try:
            ...
            if mask_squeezed.ndim == 2 and mask.ndim == 2:
                skipped += 1
                continue
            ...
            np.save(f, mask_2d)
            converted += 1
            print(f"  [OK] {f.name}: {original_shape} -> {mask_2d.shape}")
